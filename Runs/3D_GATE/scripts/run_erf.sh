...
#
# Options:
#   -c, --case=NAME       Input case name (default: sdm_amsu)
...
# Generate input file by merging base and override
BASE_FILE="$INPUTS_DIR/templates/base.inputs"
OVERRIDE_FILE="$INPUTS_DIR/templates/overrides/${CASE}.conf"

if [[ ! -f "$BASE_FILE" ]]; then
    error "Base input file not found: $BASE_FILE"
fi
...
debug "Using base file: $BASE_FILE"
debug "Using override file: $OVERRIDE_FILE"

INP="inputs_${CASE}"
info "Generating input file for case: $CASE"

# Merge base and override files
"$SCRIPT_DIR/generate_inputs.sh" --base="$BASE_FILE" --output-dir="$WORKDIR" "$CASE" "$OVERRIDE_FILE" > /dev/null 2>&1

# Verify the input file was created
if [[ ! -f "$WORKDIR/$INP" ]]; then
    error "Failed to generate input file: $WORKDIR/$INP"
fi

# Create symlink to original input_sounding file
if [[ -n "$ERF_HOME" ]]; then
    SOUNDING_PATH="$ERF_HOME/Exec/DevTests/GATE/input_sounding"
