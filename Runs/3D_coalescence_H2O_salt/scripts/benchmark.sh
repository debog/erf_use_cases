#!/bin/bash
#
# ERF benchmark comparison script
...
PLOTS_DIR="$ROOT_DIR/plots/benchmark"
...
run_benchmark_comparison() {
    ...
    sed -e "s|{root_dir}|$ROOT_DIR|g" \
        -e "s|{benchmark_dir}|$BENCHMARK_DIR|g" \
        -e "s|{platform}|$platform|g" \
        -e "s|{output_dir}|$output_dir|g" \
        -e "s|{output_format}|$output_format|g" \
        -e "s|{case_name}|$case_name|g" \
        -e "s|{kernel}|$kernel|g" \
        "$SCRIPT_DIR/benchmark_template.py" > "$script_path"
    ...
    info "Running Python to generate benchmark comparison plots"
    info "  Case: $case_name"
    info "  Kernel: $kernel"
    python "$script_path"
}
...
run_benchmark_with_kernel() {
    ...
    # Debug output
    info "Running benchmark comparison with:"
    info "  kernel: $kernel_name"
    info "  kernel_dir: $kernel_dir"
    info "  case: $case_name"
    info "  platform: $platform"

    # Silently check if both run and benchmark directories exist
    run_dir="$ROOT_DIR/.run_${case_name}.${platform}.nproc00001"
    benchmark_dir="$BENCHMARK_DIR/.run_${case_name}.${platform}.nproc00001"
...
info "Done benchmark comparison"