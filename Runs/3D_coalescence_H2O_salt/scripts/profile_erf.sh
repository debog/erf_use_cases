...
SCRIPT_NAME="profile_erf.sh"


# NOTE: Using merge_inputs from common_functions.sh

# NOTE: Using get_case_overrides from common_functions.sh

# NOTE: Using usage from common_functions.sh

# NOTE: Using get_config from common_functions.sh

# NOTE: Using platform_exists from common_functions.sh

# NOTE: Using list_platforms from common_functions.sh
# We'll need to modify it later to show profiler info

# NOTE: Using list_cases and get_all_cases from common_functions.sh
...
INP=$(generate_input_file "$CASE" "$WORKDIR" "$SCRIPT_NAME")
...
run_all_cases() {
    ...
    prof "Profiling all ${#all_cases[@]} cases in $mode mode"

    for case_name in "${all_cases[@]}"; do
        prof "======================"
        prof "Profiling case: $case_name"
        prof "======================"
        ...
        WORKDIR="$ROOT_DIR/.profile_${CASE}.${PLATFORM}.$(printf "nproc%05d" "$NTASKS")"
        if [[ -d "$WORKDIR" ]]; then
            prof "Removing existing directory: $WORKDIR"
            rm -rf "$WORKDIR"
        fi
        prof "Creating working directory: $WORKDIR"
        mkdir -p "$WORKDIR"

        cd "$WORKDIR"

        # Generate input file in the run directory
        INP=$(generate_input_file "$case_name" "$WORKDIR" "$SCRIPT_NAME")
